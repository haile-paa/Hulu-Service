...
export function BookNewPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      setError(err instanceof api.ApiError ? err.message : t("bookingFailed"));
    } finally {
      setLoading(false);
    }
  }
  ...
  if (done) {
    return (
      <div style={{ padding: "60px 24px", textAlign: "center" }}>
        ...
        <h2 style={{ fontSize: 18, margin: "0 0 6px" }}>{t("bookingSent")}</h2>
  ...
  return (
    <div>
      <TopBar title={`${t("bookingWith")} — ${state?.provider?.fullName || ""}`} />
      <div style={{ padding: "8px 20px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
        <Field label={t("describeProblem")} value={description} onChange={setDescription} />
